...
def define_graph():
    ...
    state = tf.placeholder(tf.float32, [LSTM_LAYERS, 2, BATCH_SIZE, LSTM_SIZE])
    unpack = tf.unpack(state, axis = 0)
    lstm_tuple_states = tuple([tf.nn.rnn_cell.LSTMStateTuple(unpack[i][0], unpack[i][1]) for i in range(LSTM_LAYERS)])
    
    def basic_lstm():
    	return tf.nn.rnn_cell.BasicLSTMCell(LSTM_SIZE)
    lstm_cell = tf.nn.rnn_cell.MultiRNNCell([basic_lstm() for _ in range(LSTM_LAYERS)])
    
    values, state = tf.nn.dynamic_rnn(lstm_cell, input_data, initial_state = lstm_tuple_states)
	
    logits = tf.layers.dense(values[:,-1,:], OUTPUT_SIZE, activation = tf.nn.relu)
